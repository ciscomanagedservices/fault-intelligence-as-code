...
> **Vault location for this project:** The vault root is `kb/`. All vault paths in this skill (`wiki/hot.md`, `wiki/index.md`, `.raw/`, etc.) must be prefixed with `kb/` when accessed from the workspace root. For example, `wiki/hot.md` → `kb/wiki/hot.md`, `.raw/` → `kb/.raw/`.

> **Cross-project awareness:** This skill file lives in the kb-wiki skill directory. All vault paths (`wiki/hot.md`, `wiki/index.md`, etc.) are relative to the **vault root** (`kb/`), NOT the workspace root or this skill directory. Identify the workspace folder before reading any files.
...
2b. **If the question maps to a specific content folder** (e.g., "what mentoring evidence do we have?" → `wiki/mentoring/`, "show impact data" → `wiki/impact/`), read that folder's `_index-*.md` sub-index to identify all relevant individual pages within it. Prefer this over generic index scanning for domain-scoped questions.
...
2. Identify all relevant sections (concepts, entities, sources, comparisons, and any custom content folders). For each custom content folder (any `wiki/*/` folder with an `_index-*.md`), read its sub-index to discover domain-specific pages — do not assume all relevant pages are in the three default folders.
...
## Folder Sub-Index Format

Each content folder has an `_index-[folder].md` for focused lookups:
...
Use sub-indexes when the question is scoped to one folder. Avoid reading the full master index for narrow queries.
