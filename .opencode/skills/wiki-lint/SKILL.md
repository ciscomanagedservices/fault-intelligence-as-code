...
> **Vault location for this project:** The vault root is `kb/`. All vault paths in this skill (`wiki/`, `wiki/meta/`, `wiki/index.md`, etc.) must be prefixed with `kb/` when accessed from the workspace root. For example, `wiki/meta/` → `kb/wiki/meta/`.

> **Cross-project awareness:** This skill file lives in the kb-wiki skill directory. All vault paths (`wiki/`, `wiki/meta/`, `wiki/index.md`) are relative to the **vault root** (`kb/`), NOT the workspace root or this skill directory. Identify the workspace folder before scanning.
...
9. **Empty content folders**. Folders with an `_index-*.md` sub-index but zero individual content pages. These folders were defined but never populated during ingest. Suggest: re-run ingest targeting this folder, or remove it if it is no longer needed.
...
- `wiki/[folder]/`: has `_index-[folder].md` but no content pages. Suggest: re-ingest sources targeting this folder, or remove if unused.
```
...
Add one node per content folder's index page. Connect folders that have significant cross-references. Colors map to the CSS scheme: 1=blue, 2=purple, 3=yellow, 4=orange, 5=green, 6=red.
...
- Populating empty content folders (requires re-ingest; may be a large operation)
